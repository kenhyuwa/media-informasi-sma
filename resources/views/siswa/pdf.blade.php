...
<html lang="en">
  <head>
    <title>{{ ucwords(auth('siswa')->user()->nama) }}</title>
    <style type="text/css">
...
<body>
  <div id="header">
    ...
    <div class="hr"></div>
    <div class="space">
      <h2>LEMBAR HASIL BELAJAR SISWA</h2>
    </div>
  </div>
  <!-- /header -->
  <div id="footer">
      ...
      <tr>
        ...
        <td width="100">
          ...
          <center style="margin-top: -13px;">
            Mengetahui :<br/>
            {{ ucwords($kepala_sekolah->jabatan) }}<br/><br/><br/><br/><br/>
            <u>
  ...
  <div id="content">
  <center>
    <table style="margin: 5px 0px auto;border:none;">
      <tr>
        <td width="30">NIS</td><td width="1">:</td><td width="150">{{ auth('siswa')->user()->nis }}</td>
        <td width="150">KELAS</td><td width="1">:</td><td>{{ $keterangan->getKelas->kelas }}</td>
      </tr>
      <tr>
        <td width="30">NAMA</td><td width="1">:</td><td width="150">{{ ucwords(auth('siswa')->user()->nama) }}</td>
        <td width="150">TAHUN AJARAN/SEMESTER</td><td width="1">:</td><td>{{ $keterangan->getTahunAjar->tahun }}/{{ strtoupper($semester) }}</td>
      </tr>
    </table>
  </center>
    <table border="1" cellspacing="0" style="margin-top: 20px;">
      <thead>
        <tr>
          ...
          <th width="50">Tugas 1</th>
          <th width="50">Tugas 2</th>
          <th width="50">UTS</th>
          <th width="50">UAS</th>
        </tr>
      </thead>
      ...
      @foreach($nilai_siswa as $nilai)
        <tr>
          ...
          <td>{{ ucwords($nilai->getMatpel->matpel) }}</td>
          <!-- <td>
          <center>
             <?php 
